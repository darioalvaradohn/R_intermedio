...
[Volver Arriba](GLM_Poisson.html)
...
Si nuestro salario es de $1000 y nos dicen que el siguiente mes recibiremos un aumento por un factor de 1.5:
...
Si tenemos 10 larvas por unidad de muestreo cuando no se aplica insecticida, ¿cuál será la cantidad de larvas que podríamos encontrar cuando si se aplicó insecticida:
...
- Aparecen valores de **desviación nula** que indica cuánta variabilidad hay en los datos antes de incluir las variables explicativas, y nos sirve como punto de referencia; y la **desviación residual** que indica cuánta variabilidad no es explicada por el modelo y qué tan bien se ajustan las predicciones a los datos observados. Entre más baja la desviación residual respecto a la desviación nula, mejor es el ajuste.

- También aparece el valor de AIC del modelo en caso de realizar una selección de modelos.
...
En el gráfico observamos que el intercepto del modelo indica la media de la cantidad de larvas cuando no se aplicó insecticida, y el punto cuando si se aplicó insecticida también indica la media de la cantidad de larvas. 

Para determinar esa media con los datos de la salida del modelo tenemos que multiplicar el intercepto por el factor de disminución:
...
La prueba también nos da el resultado de forma textual en la parte inferior: "Overdispersion detected"
...
Cuándo utilizar cada tipo de corrección:

- quasipoisson: 

  - cuando la sobredispersión es moderada
  
  - cuando no se requiera hacer selección de modelos con base en AIC
                
- binomial negativa: 

  - cuando la sobredispersión es muy fuerte
  
  - cuando se realizará selección de modelos con base en AIC
  
  - cuando el modelo tenga inflación de ceros
