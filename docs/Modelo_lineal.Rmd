...
La fórmula del modelo para predecir un valor sería:

$$
Largo del pico = 27.1507 + 4.0033 * Masacorporal
$$
...
```{r}
PredLargoPico <- function(Peso){
  27.1507 + 4.0033 * Peso
}
```

```{r}
PredLargoPico(Peso = 3)
```
...
Si tuviesemos un tamaño de muestra pequeño, podemos usar la función `AICtab()` del paquete `bbmle` y especificamos el argumento `base = TRUE` para que nos incluya el resultado de AICc (solo si el tamaño de muestra es pequeño).
