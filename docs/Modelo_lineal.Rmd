...
Un modelo lineal general está basado en una regresión lineal simple y nos sirve para determinar el efecto que tiene una variable predictora numérica sobre una variable respuesta numérica (Algo que no puede hacer un ANOVA), también nos sirve para evaluar si hay diferencias de una variable respuesta entre los niveles de una variable predictora categórica (similar al ANOVA).
...
```{r, message=FALSE, warning=FALSE, echo=FALSE, size= 60%}
ggplot(p, aes(peso_kg, flipper_length_mm)) +
  geom_point() +
  geom_lm() +
...
plot_model(modelo2, type = "pred",
           title = "Diferencias de longitud de aleta (mm) en relación a la Especie",
...
ggplot(medias, aes(species, mean)) +
  ...
  geom_errorbar(aes(ymin = liIC95,
                    ymax = lsIC95),
                width = 0.05) +
  labs(title = "Diferencias de longitud de aleta (mm) en relación a la Especie",
...
```{r}
ch <- check_heteroscedasticity(m7)
ch
```
...
Podemos graficar el modelo con la función `plot_model()` del paquete `sjPlot`:

```{r}
plot_model(modelo7, 
           type = "int", # argumento para graficar la interacción entre las variables
           title = "Longitud de la aleta en relación a la especie y la masa corporal de los pingüinos",
           axis.title = c("Especie", "Longitud de la aleta (mm)"),
           colors = "Set1")
...
ggplot(g, aes(x, predicted)) +
    ...
    geom_errorbar(aes(ymin = conf.low,
                  ...
                  width = 0.1) +
    theme_bw() +
  labs(title = "Longitud de la aleta en relación a la especie y masa corporal de los pingüinos",
       x = "Especie", 
...
```{r}
estimate_contrasts(modelo7, "species")
```

También podemos hacerlo de la siguiente manera:

Primero calculamos las medias de cada especie (en este caso serían las medias de la longitud de aleta) con la función `emmeans()` del paquete `emmeans` y se especifica la variable para la que se quiere ver las medias.

```{r}
medias <- emmeans(modelo7, "species")
medias
```
