...
La limpieza y manejo de datos son procesos fundamentales en el análisis de datos. A menudo, los datos en su forma original están desordenados, incompletos o mal estructurados, lo que dificulta su análisis. Por eso, antes de realizar cualquier análisis, es crucial limpiarlos y organizarlos correctamente.

La **limpieza de datos** implica identificar y corregir o eliminar errores y problemas en los datos para asegurar que estén en un formato adecuado para su análisis. Esto incluye:

- Eliminar o manejar valores faltantes (NA): Los datos pueden tener valores ausentes o nulos que deben ser tratados, ya sea eliminándolos, reemplazándolos por un valor de referencia (como la media) o interpolándolos.

- Corregir errores tipográficos o inconsistencias: Pueden ocurrir errores en la entrada de datos, como nombres escritos incorrectamente, valores numéricos erróneos o fechas mal formateadas.

- Eliminar duplicados: En muchos casos, los datos pueden contener registros duplicados que necesitan ser eliminados para evitar sesgos o distorsiones en los análisis.

- Conversión de tipos de datos: Es posible que se necesite convertir variables a su tipo correcto (por ejemplo, de texto a número o de fecha a formato correcto).

- Manejo de valores atípicos (outliers): Los valores atípicos pueden ser errores o valores válidos que deben ser manejados de manera especial para evitar que afecten el análisis.

El **manejo de datos** se refiere a cómo organizamos, transformamos y preparamos los datos para el análisis. Algunas actividades comunes son:

- Transformación de datos:

  - Reformatear datos: Cambiar el formato de los datos, como pivotar una tabla o dividir una columna en varias.

  - Escalar y normalizar: Ajustar los datos para que estén en una escala similar, especialmente en análisis estadísticos o modelos de machine learning.

  - Creación de nuevas variables: Derivar nuevas variables a partir de las existentes.

  - Filtrar y seleccionar datos: Elegir un subconjunto de datos que cumplan con ciertos criterios. 

- Agrupar y resumir: Agrupar los datos por ciertas categorías y calcular estadísticas agregadas, como la media, la mediana o la suma.

- Ordenar los datos: Organizar los datos de acuerdo con una o varias variables para facilitar el análisis o la visualización.

<br>

[Volver arriba](Limpieza.html)
...
#### Transformar variables caracter a factor

- `factor()` o `as.factor()` pueden utilizarse para sobreescribir una tabla y cambiar las variables de tipo caracter a tipo factor

```{r}
medidas$sexo <- factor(medidas$sexo)
head(medidas)
```

```{r}
medidas$sexo <- as.factor(medidas$sexo)
head(medidas)
```

Si queremos transformar todas las variables de tipo caracter a tipo factor en nuestra tabla, podemos utilizar la función `mutate_if()` del paquete `dplyr` y asignarle las condiciones:

```{r}
medidas <- mutate_if(medidas, is.character, as.factor)
head(medidas)
```
...
En nuestros datos podemos tener duplicados por diferentes razones y dependiendo de ese contexto será necesario eliminarlos o simplemente verificarlos:

- Error de digitación = eliminar duplicados

- Réplicas experimentales = verificar, no eliminar

- Datos de campo = revisar columnas clave (id, sitio, fecha)

```{r}
# Duplicado simulado
medidas <- add_row(medidas, sexo = "M", altura = 156, peso = 98, edad = 29, IMC = 40.27, Comp_corp = "Obesidad")
```

Podemos ver si existen duplicados:

```{r}
duplicated(medidas)
```

Devuelve`TRUE` para las filas repetidas (excepto la primera vez que aparece).

Podemos contar cuántos duplicados tenemos:

```{r}
sum(duplicated(medidas))
```

Podemos ver todas las filas duplicadas:

```{r}
medidas[duplicated(medidas), ]
```

```{r}
filter(medidas, duplicated(medidas))
```

Podemos ver todas las copias (incluida la primera):

```{r}
medidas[duplicated(medidas) | duplicated(medidas, fromLast = TRUE), ]
```

Eliminar duplicados:

```{r}
medidas_sin_dup <- unique(medidas)
```

o

```{r}
medidas_sin_dup <- medidas[!duplicated(medidas), ]
```
...
medidas <- rename(medidas, 
                  condicion_corporal = Comp_corp) # colocamos el nuevo nombre de la variable y con el "=" indicamos la variable a la que le cambieremos el nombre
head(medidas)
```
...
- También podemos renombrar las variables de forma automática con la función `clean_names()` del paquete `janitor`:

```{r}
nombresnuevos <- clean_names(medidas, case = "snake") # El argumento `case` hace referencia al formato de los caracteres y a la unión/separación de dos o más palabras. Puede ser: "snake", "small_camel", "big_camel", "screaming_snake", "parsed", "mixed", "lower_upper", "upper_lower", "swap", "all_caps", "lower_camel", "upper_camel", "internal_parsing", "none", "flip", "sentence", "random", "title".
head(nombresnuevos)
```
...
#### Renombrar observaciones de una variable 

- `recode()` del paquete `dplyr` para cambiar el nombre de una o más observaciones en la tabla de datos:

```{r}
medidas$Sexo <- recode(medidas$Sexo, "M" = "Masculino", "F" = "Femenino")
distinct(medidas, Sexo)
```
...
formatoancho <- pivot_wider(medidas, 
                     names_from = Condicion_Corporal, # indicamos la variable que contiene los niveles que pasarán a ser columnas
